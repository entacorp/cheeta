...
public class LiteralVisitor extends PCREBaseVisitor<Literals> {
	...
	@Override
	public Literals visitParse(PCREParser.ParseContext ctx) {
		return visitAlternation(ctx.alternation());
	}

	@Override
	public Literals visitAlternation(PCREParser.AlternationContext ctx) {
		List<Literals> elementLiterals = new ArrayList<>();
		for (PCREParser.ExprContext expr: ctx.expr())
	...
	@Override
	public Literals visitExpr(PCREParser.ExprContext ctx) {
		List<Literals> elementLiterals = new ArrayList<>();
		
		for (PCREParser.ElementContext element: ctx.element()) {
			Literals atomLiterals = visitAtom(element.atom());
			if (atomLiterals != null) {
				PCREParser.QuantifierContext quantifier = element.quantifier();
				if (quantifier != null) {
	...
	@Nullable
	@Override
	public Literals visitAtom(PCREParser.AtomContext ctx) {
		if (ctx.literal() != null) {
			...
			for (PCREParser.Cc_atomContext cc_atom: ctx.character_class().simple_character_class().cc_atom()) {
