...
public class EyesCountCriteria extends Criteria<Issue> {
	...
	public Predicate getPredicate(@Nullable ProjectScope projectScope, CriteriaQuery<?> query, From<Issue, Issue> from, CriteriaBuilder builder) {
		Path<Integer> attribute = from.get(Issue.PROP_EYES_COUNT);
	...
	public boolean matches(Issue issue) {
		if (operator == IssueQueryLexer.Is)
			return issue.getEyesCount() == value;
		else if (operator == IssueQueryLexer.IsNot)
			return issue.getEyesCount() != value;
		else if (operator == IssueQueryLexer.IsGreaterThan)
			return issue.getEyesCount() > value;
	...
	public String toStringWithoutParens() {
		return quote(Issue.NAME_EYES_COUNT) + " " 
