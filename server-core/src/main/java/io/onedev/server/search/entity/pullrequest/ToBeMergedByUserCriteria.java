...
public class ToBeMergedByUserCriteria extends Criteria<PullRequest> {

	private static final long serialVersionUID = 1L;

	private final User user;
	
	public ToBeMergedByUserCriteria(User user) {
		this.user = user;
	}
	...
	private Criteria<PullRequest> getCriteria(User user) {
		return new AndCriteria<>(new ReadyToMergeCriteria(), new AssignedToUserCriteria(user));
	}
