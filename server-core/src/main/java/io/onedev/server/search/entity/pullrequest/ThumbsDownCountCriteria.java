...
public class ThumbsDownCountCriteria extends Criteria<PullRequest> {
	...
	public ThumbsDownCountCriteria(int value, int operator) {
	...
	public Predicate getPredicate(@Nullable ProjectScope projectScope, CriteriaQuery<?> query, From<PullRequest, PullRequest> from, CriteriaBuilder builder) {
		Path<Integer> attribute = from.get(PullRequest.PROP_THUMBS_DOWN_COUNT);
	...
	public boolean matches(PullRequest request) {
		if (operator == PullRequestQueryLexer.Is)
			return request.getThumbsDownCount() == value;
		else if (operator == PullRequestQueryLexer.IsNot)
			return request.getThumbsDownCount() != value;
		else if (operator == PullRequestQueryLexer.IsGreaterThan)
			return request.getThumbsDownCount() > value;
	...
	public String toStringWithoutParens() {
		return quote(PullRequest.NAME_THUMBS_DOWN_COUNT) + " " 
