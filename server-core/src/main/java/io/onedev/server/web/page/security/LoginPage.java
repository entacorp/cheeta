...
import static io.cheeta.server.web.page.security.SsoProcessPage.MOUNT_PATH;
import static io.cheeta.server.web.page.security.SsoProcessPage.STAGE_INITIATE;
...
import io.cheeta.server.web.component.user.twofactorauthentication.TwoFactorAuthenticationSetupPanel;
import io.cheeta.server.web.page.simple.SimpleCssResourceReference;
...
public class LoginPage extends SimplePage {
	...
	protected void onInitialize() {
		...
		Form<?> form = new Form<Void>("form") {
			...
			protected void onSubmit() {
				...
				try {
					...
					var user = (User) Cheeta.getInstance(PasswordAuthenticatingRealm.class).getAuthenticationInfo(token);
					if (user.isEnforce2FA()) {
						if (user.getTwoFactorAuthentication() != null) {
							subTitle = _T("Two-factor authentication is enabled. Please input passcode displayed on your TOTP authenticator. If you encounter problems, make sure time of Cheeta server and your device running TOTP authenticator is in sync");
		...
		var ssoProviderService = Cheeta.getInstance(SsoProviderService.class);
	...
	private void afterLogin(User user) {
		RememberMeManager rememberMeManager = Cheeta.getInstance(RememberMeManager.class);
		if (rememberMe) {
	...
	protected String getTitle() {
		return _T("Sign In To") + " " + Cheeta.getInstance(SettingService.class).getBrandingSetting().getName();
	}
