...
import io.cheeta.server.web.component.issue.pullrequests.IssuePullRequestsPanel;
					...
					protected Component newDeleteLink(String componentId) {
						return new AjaxLink<Void>(componentId) {
							...
							public void onClick(AjaxRequestTarget target) {
								Cheeta.getInstance(IssueService.class).delete(getIssue());
								var oldAuditContent = VersionedXmlDoc.fromBean(getIssue()).toXML();
								Cheeta.getInstance(AuditService.class).audit(getIssue().getProject(), "deleted issue \"" + getIssue().getReference().toString(getIssue().getProject()) + "\"", oldAuditContent, null);
