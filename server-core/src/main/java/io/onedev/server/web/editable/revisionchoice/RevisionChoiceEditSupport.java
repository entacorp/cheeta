...
public class RevisionChoiceEditSupport implements EditSupport {
	...
	public PropertyContext<?> getEditContext(PropertyDescriptor descriptor) {
        ...
        if (propertyGetter.getAnnotation(RevisionChoice.class) != null) {
        	if (propertyGetter.getReturnType() == String.class) {
        		return new PropertyContext<String>(descriptor) {
					...
					public PropertyEditor<String> renderForEdit(String componentId, IModel<String> model) {
						return new RevisionChoiceEditor(componentId, descriptor, model);
					}
        			
        		};
        	} else {
        		throw new RuntimeException("Annotation 'RevisionChoice' should be applied to property with type 'String'");
        	}
        } else {
            return null;
        }
