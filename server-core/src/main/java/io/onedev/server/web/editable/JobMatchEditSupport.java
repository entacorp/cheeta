...
import io.cheeta.server.web.behavior.JobMatchBehavior;
...
public class JobMatchEditSupport implements EditSupport {
	...
	public PropertyContext<?> getEditContext(PropertyDescriptor descriptor) {
		...
		JobMatch jobMatch = propertyGetter.getAnnotation(JobMatch.class);
        if (jobMatch != null) {
    		...
    		return new PropertyContext<String>(descriptor) {
				...
				public PropertyEditor<String> renderForEdit(String componentId, IModel<String> model) {
		        	return new StringPropertyEditor(componentId, descriptor, model)
		        			.setInputAssist(new JobMatchBehavior(jobMatch.withProjectCriteria(), jobMatch.withJobCriteria()));
				}
