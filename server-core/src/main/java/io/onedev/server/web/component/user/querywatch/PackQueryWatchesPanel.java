...
class PackQueryWatchesPanel extends GenericPanel<User> {
    ...
    private final IModel<List<QueryInfo>> queryInfosModel = new LoadableDetachableModel<>() {
        ...
        protected List<QueryInfo> load() {
            ...
            var packSetting = getSettingService().getPackSetting();
                    ...
                    for (IModel<QueryInfo> each: selectionColumn.getSelections()) {
                        ...
                        if (queryInfo.projectId == null) {
                             ...
                             if (getPage() instanceof UserPage)
                                auditService.audit(null, "unwatched pack query \"" + queryInfo.name + "\" in account \"" + getUser().getName() + "\"", null, null);
                        } else {
                             for (var personalization: getUser().getPackQueryPersonalizations()) {
                                 if (personalization.getProject().getId().equals(queryInfo.projectId)) {
                                     ...
                                     getPackQueryPersonalizationService().createOrUpdate(personalization);
                                     if (getPage() instanceof UserPage)
                                        auditService.audit(null, "unwatched pack query \"" + queryInfo.name + "\" in account \"" + getUser().getName() + "\" in project \"" + personalization.getProject().getPath() + "\"", null, null);
                                }
                             }
                        }
                     }
    ...
    private PackQueryPersonalizationService getPackQueryPersonalizationService() {
        return Cheeta.getInstance(PackQueryPersonalizationService.class);
    }
