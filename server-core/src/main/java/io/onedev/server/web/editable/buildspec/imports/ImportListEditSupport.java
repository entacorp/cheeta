...
public class ImportListEditSupport implements EditSupport {
			...
			if (elementClass == Import.class) {
				return new PropertyContext<List<Serializable>>(descriptor) {
						...
						return new PropertyViewer(componentId, descriptor) {
							...
							protected Component newContent(String id, PropertyDescriptor propertyDescriptor) {
								if (model.getObject() != null) {
									return new ImportListViewPanel(id, model.getObject());
								} else { 
					...
					public PropertyEditor<List<Serializable>> renderForEdit(String componentId, IModel<List<Serializable>> model) {
						return new ImportListEditPanel(componentId, descriptor, model);
					}
