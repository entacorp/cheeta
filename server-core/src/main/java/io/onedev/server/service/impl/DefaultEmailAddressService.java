...
@Singleton
public class DefaultEmailAddressService extends BaseEntityService<EmailAddress> implements EmailAddressService {
	...
	public void create(EmailAddress emailAddress) {
		...
		if (!emailAddress.isVerified() && settingService.getMailConnector() != null) {
			Long addressId = emailAddress.getId();
			sessionService.runAsyncAfterCommit(() -> sendVerificationEmail(load(addressId)));
		}
	}
	...
	public void sendVerificationEmail(EmailAddress emailAddress) {
		...
		String template = settingService.getEmailTemplates().getEmailVerification();
		...
		mailService.sendMail(Arrays.asList(emailAddress.getValue()),
