...
@Singleton
public class DefaultBatchWorkExecutionService implements BatchWorkExecutionService, Runnable {

	private static final Logger logger = LoggerFactory.getLogger(DefaultBatchWorkExecutionService.class);
			...
			for (Iterator<Map.Entry<BatchWorker, Works>> it = works.entrySet().iterator(); it.hasNext();) {
				...
				if (works.working.isEmpty()) {
					...
					if (!works.working.isEmpty()) {
						...
						workExecutionService.submit((int) priority, new Runnable() {
							...
							public void run() {
								try {
									...
									synchronized (DefaultBatchWorkExecutionService.this) {
										works.working.clear();
										DefaultBatchWorkExecutionService.this.notify();
									}
								}
							}
