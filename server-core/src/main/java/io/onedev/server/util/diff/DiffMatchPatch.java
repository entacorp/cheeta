...
/*
 * Functions for diff, match and patch. Computes the difference between two
 * texts to create a patch. Applies the patch onto another text, allowing for
 * errors.
 * 
...
public class DiffMatchPatch {
		...
		for (int d = 0; d < max_d; d++) {
			...
			for (int k2 = -d + k2start; k2 <= d - k2end; k2 += 2) {
				...
				while (x2 < text1_length
						&& y2 < text2_length
						&& text1.charAt(text1_length - x2 - 1) == text2.charAt(text2_length - y2
								- 1)) {
					x2++;
					y2++;
				}
	 ...
	 * @return LinkedList of Diff objects.
	 */
	private LinkedList<Diff> diff_bisectSplit(String text1, String text2, int x, int y,
			long deadline) {
	...
	protected int diff_commonOverlap(String text1, String text2) {
		...
		while (true) {
			...
			length += found;
			if (found == 0
					|| text1.substring(text_length - length).equals(text2.substring(0, length))) {
				best = length;
				length++;
			}
		}
	...
	public void diff_cleanupEfficiency(LinkedList<Diff> diffs) {
		...
		while (thisDiff != null) {
			if (thisDiff.operation == Operation.EQUAL) {
				 ...
				 */
				if (lastequality != null
						&& ((pre_ins && pre_del && post_ins && post_del) || ((lastequality.length() < Diff_EditCost / 2) && ((pre_ins
								? 1 : 0) + (pre_del ? 1 : 0) + (post_ins ? 1 : 0) + (post_del ? 1
									: 0)) == 3))) {
					// System.out.println("Splitting: '" + lastequality + "'");
					// Walk back to offending equality.
					while (thisDiff != equalities.lastElement()) {
							...
							if (commonlength != 0) {
								...
								thisDiff.text =
										text_insert.substring(text_insert.length() - commonlength)
												+ thisDiff.text;
								text_insert =
										text_insert.substring(0, text_insert.length()
												- commonlength);
								text_delete =
										text_delete.substring(0, text_delete.length()
												- commonlength);
								pointer.previous();
							}
						}
	...
	public String diff_prettyHtml(LinkedList<Diff> diffs) {
		...
		for (Diff aDiff : diffs) {
			String text =
					aDiff.text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
							.replace("\n", "&para;<br>");
		...
		for (Diff aDiff : diffs) {
			...
			switch (aDiff.operation) {
				case INSERT:
					...
					postpatch_text =
							postpatch_text.substring(0, char_count2) + aDiff.text
				...
				case EQUAL:
					...
					if (aDiff.text.length() >= 2 * Patch_Margin) {
						...
						if (!patch.diffs.isEmpty()) {
							...
							// Update prepatch text & pos to reflect the
							// application of the
	...
	public Object[] patch_apply(LinkedList<Patch> patches, String text) {
		...
		for (Patch aPatch : patches) {
			...
			if (text1.length() > this.Match_MaxBits) {
				...
				if (start_loc != -1) {
					end_loc =
							match_main(text, text1.substring(text1.length() - this.Match_MaxBits),
			...
			if (start_loc == -1) {
				...
				if (text1.equals(text2)) {
					...
					text =
							text.substring(0, start_loc) + diff_text2(aPatch.diffs)
						...
						for (Diff aDiff : aPatch.diffs) {
							if (aDiff.operation != Operation.EQUAL) {
								...
								if (aDiff.operation == Operation.INSERT) {
									// Insertion
									text =
											text.substring(0, start_loc + index2) + aDiff.text
